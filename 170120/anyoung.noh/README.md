...
공인인증서는 이를 저장하는 저장장치마다 독립적인 비밀번호를 사용한다. 따라서 저장장치의 분실/유출 상황이 발생했을 때 다른 저장장치에 있는 공인인증서의 비밀번호를 바꿔도 분실된 저장장치의 공인인증서에는 영향을 주지 않는다.(이미 분실된 공인인증서에 SEED로 비밀번호가 저장되어있음) 이 경우에는 인증기관에 신고를 한 후 해당 인증서를 폐기 후 재발급 해야한다. 
